...
enum UpdateExternalResourceAction {
    static func run() {
        Task { @MainActor in
            let provider = await ApiClient.shared.requestExternalProviderNames()
            ...
            for name in provider.proxies {
                if await ApiClient.shared.updateProvider(name: name, type: .proxy) {
                    successCount += 1
                } else {
                    fails.append(name)
                }
            ...
            for name in provider.rules {
                if await ApiClient.shared.updateProvider(name: name, type: .rule) {
                    successCount += 1
                } else {
                    fails.append(name)
                }
