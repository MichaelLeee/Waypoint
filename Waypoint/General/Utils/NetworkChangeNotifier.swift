...
class NetworkChangeNotifier {
    ...
    private static func startProxiesWatch() {
        ...
        if let dynamicStore = SCDynamicStoreCreate(kCFAllocatorDefault, "org.waypnt.proxy.networknotification" as CFString, changed, dcAddress) {
    ...
    private static func startIPChangeWatch() {
        ...
        if let dynamicStore = SCDynamicStoreCreate(kCFAllocatorDefault, "org.waypnt.ipv4.networknotification" as CFString, changed, dcAddress) {
    ...
    @MainActor static func isCurrentSystemSetToWaypoint(looser: Bool = false) -> Bool {
    ...
    // Reads ConfigManager (MainActor); all callers are main-thread UI paths.
    @MainActor static func hasInterfaceProxySetToWaypoint() -> Bool {
        ...
        if let prefRef = SCPreferencesCreate(nil, "Waypoint" as CFString, nil),
    ...
    static func getPrimaryInterface() -> String? {
        let store = SCDynamicStoreCreate(nil, "Waypoint" as CFString, nil, nil)
        if store == nil {
            return nil
        }
